import os
from os import getenv

from dotenv import load_dotenv

if os.path.exists("local.env"):
    load_dotenv("local.env")

que = {}
SESSION_NAME = getenv("SESSION_NAME", "session")
BOT_TOKEN = getenv("BOT_TOKEN"6690642431:AAGSF6SvAuOx-Wu6ztRExET6e85DcTroTz8")
BOT_NAME = getenv("BOT_NAME"alya music bot)
UPDATES_CHANNEL = getenv("UPDATES_CHANNEL", "pika sub")
BG_IMAGE = getenv("BG_IMAGE", "https://telegra.ph/file/9b13ea3ce046a1a5c8098.png")
admins = {}
API_ID = int(getenv("API_ID"20718334"))
API_HASH = getenv("API_HASH"4e81464b29d79c58d0ad8a0c55ece4a5")
BOT_USERNAME = getenv("BOT_USERNAME")
ASSISTANT_NAME = getenv("ASSISTANT_NAME", "obito")
OWNER_NAME = getenv("OWNER_NAME", " ๏ 𝐎ʙɪᴛᴏ ᴜᴄʜɪʜᴀ ๏")
SUPPORT_GROUP = getenv("SUPPORT_GROUP", "Pika Fan Discussion")
PROJECT_NAME = getenv("PROJECT_NAME", "VCPlayBot2.0")
SOURCE_CODE = getenv("SOURCE_CODE", "github.com/QueenArzoo/VCPlayBot")
DURATION_LIMIT = int(getenv("DURATION_LIMIT", "7"))
ARQ_API_KEY = getenv("ARQ_API_KEY", None)
PMPERMIT = getenv("PMPERMIT", None)
LOG_GRP = getenv("LOG_GRP", None)
COMMAND_PREFIXES = list(getenv("COMMAND_PREFIXES", "/ !").split())

SUDO_USERS = list(map(int, getenv("SUDO_USERS", "5096741943").split()))
